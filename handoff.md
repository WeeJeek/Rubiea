...
- **Confirmed:** Mobile opens directly on the first or URL-selected complete Moment. A current-title plus `All stories / Alle verhalen` control opens a bottom sheet for switching; the sheet is not a reading gate, and selection closes it, updates the independent URL, and returns to the same single-column reading surface.
- **Open:** Final desktop visual composition, anonymous discovery and privacy boundaries, authorised Stone archive card, implementation, and interaction tests remain under `GEM-009`.
