...
- **Confirmed:** The random entry is always visible as `Encounter an anonymous Moment / Ontmoet een anoniem moment`. Pointer, touch, and keyboard activation each select one eligible anonymous Moment; discovery does not depend on hover or a timed ambient event.
...
- **Open:** Final desktop visual composition, random-entry plus back/repeat-control appearance, anonymous URL/search/zero-story/redaction boundaries, authorised Stone archive card, implementation, and interaction tests remain under `GEM-009`.
