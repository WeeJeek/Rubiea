...
- 匿名 Moment 不进入桌面左侧目录或手机 `All stories / Alle verhalen` 抽屉，也不在这两处显示其标题或一句概括。页面始终显示 `Encounter an anonymous Moment / Ontmoet een anoniem moment`；访客点击、触摸或用键盘激活后随机发现一篇符合发布与脱敏条件的匿名故事。入口不是悬浮才出现的彩蛋，也不要求等待或抢点定时事件。其最终造型、打开位置、再次随机、URL、搜索收录、无故事状态与脱敏规则仍待后续定稿。随机展示只表达偶然相遇，不构成隐私保护。
