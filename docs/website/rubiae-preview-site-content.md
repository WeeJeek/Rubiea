...
- 匿名 Moment 不进入桌面左侧目录或手机 `All stories / Alle verhalen` 抽屉，也不在这两处显示其标题或一句概括。页面始终显示 `Encounter an anonymous Moment / Ontmoet een anoniem moment`；访客点击、触摸或用键盘激活后，桌面右侧展台或手机单列阅读面原位替换为一篇符合发布与脱敏条件的匿名故事。不弹出长文模态框，不经过独立中间页。桌面普通目录保持可见但不为匿名故事新增或错误高亮条目；手机抽屉保持不变。明确的 `Back to stories / Terug naar verhalen` 恢复触发随机入口前所读的非匿名 Moment。匿名全文末尾显示 `Encounter another anonymous Moment / Ontmoet nog een anoniem moment`；只有访客主动激活才在原阅读面换一篇，不因读完、滚动或等待自动继续。至少有两篇合格匿名故事时不立即重复当前故事，连续遇见也始终保留最初的非匿名返回目标。入口不是悬浮才出现的彩蛋，也不要求等待或抢点定时事件。其最终造型、返回与再次遇见控件的精确位置、URL、搜索收录、零篇／单篇状态与脱敏规则仍待后续定稿。随机展示只表达偶然相遇，不构成隐私保护。
