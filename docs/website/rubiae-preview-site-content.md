...
- 手机端首次进入即显示第一篇或当前 URL 对应的完整故事。故事上方显示当前标题与 `All stories / Alle verhalen`；点击后从底部打开标题和一句概括组成的故事抽屉。选择一项后抽屉关闭、独立 URL 更新，并在同一单列页面显示对应物品／影像或空展台与完整故事。抽屉不是阅读前置步骤，不使用横向滑动标题条；其收录范围不提前决定仍待定的匿名随机展示方案。
