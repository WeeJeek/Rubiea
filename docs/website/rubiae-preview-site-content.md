...
已批准的桌面视觉密度只使用三个主要摄影时刻：生活化首图、中性光整体、微距与侧面组合。毫米参照、报告与视频仍为必备证据，但在首层页面收束为紧凑的 `Further evidence / Meer bewijs` 入口，并可使用小型真实预览；顾客打开入口后仍能查看完整影像、报告或视频。不得把这些入口扩张为连续满宽图片、等高图片墙或传统缩略图网格。

页面叙事只表达观察与核验过程：遇见、慢看、转动观察、确认尺度、核对证据、提出问题。通过一处局部手部、少量工具、物件位移和摄影远近变化形成时间推进，不虚构顾客故事、人生节点、引语、产地或商品结论。页面以雾白和中性纸白为主，烟灰辅助，深梅与炭灰只作少量局部；摄影之间须有足够留白，形成“看一张、读一点、停一下”的节奏。
...
每颗售出宝石继续使用原 Stone URL 作为永久事实档案。只有顾客另行授权非匿名发布 Rubiae Moment，并单独授予“故事与宝石关联授权”时，Stone 档案才在事实与报告之后显示一张故事摘要卡，链接到独立 Moment 详情页；完整故事不复制进商品事实区。匿名状态永不建立公开双向链接或显示宝石编号、照片、名称、石种等关联，即使后台保留内部关联记录或已取得独立关联授权。顾客须另行批准改为非匿名发布，且关联授权仍有效，才可公开关联。没有故事授权时不创建故事页，也不显示空故事模块。撤回关联授权只移除链接和具体宝石标识，不删除仍获发布授权的故事或永久宝石档案。
...
上述证据资产要求不因首层页面减少大图而减少。三个主要摄影时刻负责页面节奏，其余证据通过紧凑入口进入完整查看；任何小型预览、裁切或组合都必须继续绑定同一实物与商品编号。

## Stories / Rubiae Moments

公开内容仍区分 Stories 聚合界面、Moment 和永久 Stone 档案，但 Stories 与 Moment 在视觉上共用一个直接可读的界面。左侧只纵览进入普通目录的非匿名故事，右侧立即显示当前 Moment 的获授权物品／影像与完整故事，不经过独立封面或摘要中间页。普通目录中的非匿名 Moment 具有独立 URL；匿名 Moment 不提供公开固定链接，也不单独进入搜索收录。非匿名 Story 与 Stone 只有取得独立的“故事与宝石关联授权”时才互相链接；匿名状态下任何 Stone 关联都只保留在内部。故事内容与商品事实始终分区呈现。
...
- `{{approved_images}}` 为空时，右侧展台保持刻意空置，故事纸面成为唯一展品。页面不得补通用宝石图、生成象征物、装饰性道具或替代照片，也不得把空置状态显示成上传失败或内容缺失。
...
- 每个进入普通目录的非匿名 Moment 使用独立 URL。匿名 Moment 没有公开可路由 URL；随机打开时保留当前非匿名 Moment URL，刷新后恢复该普通故事。匿名正文不进入 sitemap、页面 metadata 或搜索引擎索引。只有非匿名发布与故事对 Stone 的独立关联授权同时存在时，才显示对应 Stone 档案链接。完整故事只保存在 Moment 详情页，Stone 档案使用摘要卡链接，不重复全文。
...
- 点击左侧非匿名总结性标题后，展台物品／影像、署名与完整故事同步切换，同时更新到该 Moment 的独立 URL，不再出现第二次“阅读全文”点击。切换不自动播放，支持鼠标、键盘与手机触摸；普通模式可使用克制淡换和轻位移，`prefers-reduced-motion` 下直接替换。长故事自然纵向阅读，不人为拆成翻书页。
- 手机端首次进入普通 Stories 即显示第一篇或当前 URL 对应的非匿名完整故事。故事上方显示当前标题与 `All stories / Alle verhalen`；点击后从底部打开标题和一句概括组成的非匿名故事抽屉。选择一项后抽屉关闭、独立 URL 更新，并在同一单列页面显示对应物品／影像或空展台与完整故事。抽屉不是阅读前置步骤，不使用横向滑动标题条。
- 匿名 Moment 不进入桌面左侧目录或手机 `All stories / Alle verhalen` 抽屉，也不在这两处显示其标题或一句概括。至少有一篇符合发布与脱敏条件的匿名故事时，页面始终显示 `Encounter an anonymous Moment / Ontmoet een anoniem moment`；没有合格故事时，整个邀请隐藏，不显示失效按钮或空状态文案，普通 Stories 保持不变。访客点击、触摸或用键盘激活邀请后，桌面右侧展台或手机单列阅读面原位替换为一篇匿名故事。不弹出长文模态框，不经过独立中间页。匿名故事不改变当前非匿名 Moment URL，刷新后恢复普通故事；无法通过公开固定链接直接重访，也不进入搜索索引。此限制不构成保密，访客仍可能复制或截图。匿名故事统一使用英文标题 `A Rubiae Moment`、荷兰语标题 `Een Rubiae Moment`，不显示顾客或故事特有标题；署名固定为英文页 `Anonymous`、荷兰语页 `Anoniem`，不显示姓名、缩写、自选化名、城市、年龄或购买时间。匿名发布前，Rubiae 检查正文中的直接身份信息及精确日期、地点、单位、学校、顾客／订单／宝石标识和罕见经历组合，只提出不改变经历、语气或含义的最小删除或概括；顾客确认最终脱敏原文与页面预览。自动脱敏或顾客单独自检不能替代此流程。若无法在保留原意的同时降低可合理预见的识别风险，则不以匿名方式发布。匿名图片逐张清除 EXIF、IPTC、XMP、GPS、嵌入预览和原公开文件名，并检查可见身份线索；只在不改变含义或误导画面的情况下裁切或遮挡识别区域。顾客确认最终图与页面预览；无法安全处理时不用该图，使用已批准的空展台，不补通用或生成替代图。匿名状态下不显示 Stone 卡、照片、名称、编号、石种或链接；内部关联记录可以保留，但公开关联须由顾客另行批准改为非匿名发布，并同时具备故事与宝石关联授权。桌面普通目录保持可见但不为匿名故事新增或错误高亮条目；手机抽屉保持不变。明确的 `Back to stories / Terug naar verhalen` 恢复触发随机入口前所读的非匿名 Moment。匿名全文末尾显示 `Encounter another anonymous Moment / Ontmoet nog een anoniem moment`；只有访客主动激活才在原阅读面换一篇，不因读完、滚动或等待自动继续。至少有两篇合格匿名故事时不立即重复当前故事，连续遇见也始终保留最初的非匿名返回目标。只有一篇合格匿名故事时，主邀请仍可打开它；文末不显示“再遇见”或失效控件，只保留返回普通故事。入口不是悬浮才出现的彩蛋，也不要求等待或抢点定时事件。其最终造型以及返回与再次遇见控件的精确位置仍待后续定稿。随机展示只表达偶然相遇，不构成隐私保护。
- 非匿名 Moment 取得故事与宝石关联授权时，在全文结束后显示紧凑 Stone 档案卡，包含真实宝石照片、获授权公开名称或石种、商品编号、`Sold` 和档案链接。未授权或仍为匿名状态时，宝石不进入故事展台，也不穿插进正文事实。
