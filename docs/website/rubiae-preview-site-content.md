...
公开内容仍区分 Stories 聚合界面、Moment 和永久 Stone 档案，但 Stories 与 Moment 在视觉上共用一个直接可读的界面。左侧只纵览进入普通目录的非匿名故事，右侧立即显示当前 Moment 的获授权物品／影像与完整故事，不经过独立封面或摘要中间页。普通目录中的非匿名 Moment 具有独立 URL；匿名 Moment 的 URL 与搜索收录仍待定。Story 与 Stone 只有取得独立的“故事与宝石关联授权”时才互相链接；匿名发布默认不包含该授权。故事内容与商品事实始终分区呈现。
...
- 每个进入普通目录的非匿名 Moment 使用独立 URL；匿名 Moment 的 URL 仍待后续决定。只有授权允许公开关联时，才显示对应 Stone 档案链接。完整故事只保存在 Moment 详情页，Stone 档案使用摘要卡链接，不重复全文。
...
- 匿名 Moment 不进入桌面左侧目录或手机 `All stories / Alle verhalen` 抽屉，也不在这两处显示其标题或一句概括。至少有一篇符合发布与脱敏条件的匿名故事时，页面始终显示 `Encounter an anonymous Moment / Ontmoet een anoniem moment`；没有合格故事时，整个邀请隐藏，不显示失效按钮或空状态文案，普通 Stories 保持不变。访客点击、触摸或用键盘激活邀请后，桌面右侧展台或手机单列阅读面原位替换为一篇匿名故事。不弹出长文模态框，不经过独立中间页。桌面普通目录保持可见但不为匿名故事新增或错误高亮条目；手机抽屉保持不变。明确的 `Back to stories / Terug naar verhalen` 恢复触发随机入口前所读的非匿名 Moment。匿名全文末尾显示 `Encounter another anonymous Moment / Ontmoet nog een anoniem moment`；只有访客主动激活才在原阅读面换一篇，不因读完、滚动或等待自动继续。至少有两篇合格匿名故事时不立即重复当前故事，连续遇见也始终保留最初的非匿名返回目标。只有一篇合格匿名故事时，主邀请仍可打开它；文末不显示“再遇见”或失效控件，只保留返回普通故事。入口不是悬浮才出现的彩蛋，也不要求等待或抢点定时事件。其最终造型、返回与再次遇见控件的精确位置、URL、搜索收录与脱敏规则仍待后续定稿。随机展示只表达偶然相遇，不构成隐私保护。
