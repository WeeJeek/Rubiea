...
- 匿名 Moment 不进入桌面左侧目录或手机 `All stories / Alle verhalen` 抽屉，也不在这两处显示其标题或一句概括。页面始终显示 `Encounter an anonymous Moment / Ontmoet een anoniem moment`；访客点击、触摸或用键盘激活后，桌面右侧展台或手机单列阅读面原位替换为一篇符合发布与脱敏条件的匿名故事。不弹出长文模态框，不经过独立中间页。桌面普通目录保持可见但不为匿名故事新增或错误高亮条目；手机抽屉保持不变。明确的 `Back to stories / Terug naar verhalen` 恢复触发随机入口前所读的非匿名 Moment。入口不是悬浮才出现的彩蛋，也不要求等待或抢点定时事件。其最终造型、返回控件精确位置、再次随机、URL、搜索收录、无故事状态与脱敏规则仍待后续定稿。随机展示只表达偶然相遇，不构成隐私保护。
